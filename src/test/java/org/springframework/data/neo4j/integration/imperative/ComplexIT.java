	...
	void shouldLoadOnlyIdsOfRelation(@Autowired BaseNodeRepository<BaseNodeEntity> repository) {
		...
		assertThat(rootNode).isPresent()
				...
				.satisfies(root -> {
					...
					assertThat(root).extracting(NodeWithChild::getChildNodes, (as(iterable(NodeWithoutChild.class))))
							.hasSize(1)
							.first()
							.satisfies(child -> assertThat(child).extracting(NodeWithoutChild::getNodeId).isEqualTo("comp"));
				});
	}
